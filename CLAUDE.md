...
**Design System:** The UI uses a clean professional SaaS aesthetic as the default (light mode). Light mode: soft gray page background (`#f0f2f5`), white sidebar and cards with subtle shadows (`0 2px 8px rgba(0,0,0,0.06)`), sky blue accent `#0ea5e9`, system-ui sans-serif typography. Dark mode: near-black page background, cards with `linear-gradient(145deg, #0f1b34, #080f1d)`, cyan accent `#00e5ff`, cyberpunk glow effects. Theme is toggled by the user via profile page and stored in localStorage (default: light). CSS variables are defined in `theme.css` under `:root` (dark) and `[data-theme="light"]`. The sidebar (`sidebar.js`) injects a `<style>` block at page-load time for theme-aware overrides and uses CSS variables throughout — do not hardcode colors. Cybersecurity utility classes (`.mono-id`, `.terminal-label`, `.card-threat-c/h/m/l`, `.threat-pulse`, `.scan-line`, `.glitch-hover`) are preserved in `theme.css` and active in dark mode.

**Dark mode card edge highlighting (v1.8+):** In dark mode (`data-theme="dark"` on `<html>`), all `.card` elements get a subtle cyan edge via a `[data-theme="dark"] .card` rule in `theme.css` (mirrors the existing `[data-theme="light"] .card` override pattern). The three-layer shadow — `0 0 0 1px rgba(0,229,255,0.08)` (outer ring) + depth + `inset 0 1px 0 rgba(0,229,255,0.15)` (top-edge highlight) — uses `!important` so it overrides the hardcoded box-shadow values in individual HTML page `<style>` blocks. Interactive cards (`.card-hover:hover`) intensify the glow on hover. Threat-level cards (`.card-threat-c/h`) add the top-edge inset while preserving their severity left-border and existing glow. Light mode is completely unaffected (the `[data-theme="light"] .card !important` rule comes after and wins). `--border-card` updated to `rgba(0,229,255,0.22)` (was 0.12) and `--card-shadow` updated to include the ring + inset for any cards using CSS variables.

The sidebar is a shared JS component defined in `frontend/static/js/sidebar.js`. Each HTML page uses `<div id="sidebar-mount"></div>` followed by `<script src="/static/js/sidebar.js"></script>` — the script renders the full sidebar, auto-detects the active nav link, injects a `<style>` block with theme-aware CSS for the sidebar, and defines `window.applyTheme()`. Section labels are plain uppercase sans-serif (no `//` prefix). Do not duplicate sidebar HTML across pages; edit `sidebar.js` for any sidebar changes.
...
The application detail page (`/app-detail.html`) has a **Remediation Card** between the top info/score grid and the finding-stats strip. If no plan exists it shows "No AI remediation plan yet" + a "Plan Remediation" button. If a plan exists it shows the status badge, model pill, finding count, a ~40-word summary of `ai_plan`, a "View full plan ↓" button (smooth-scrolls to the bottom section), and an "Open PR" button (links to `pr_url` if a PR exists, otherwise calls the execute endpoint). A **Remediation Section** below the tabs card renders the full `ai_plan` in a scrollable `<pre>` with the same "Open PR" button. Both are populated by `loadRemediationCard()` which fetches `GET /api/v1/remediation?application_id={id}&page_size=1`. The card and section refresh automatically after plan generation completes.

The application detail page also has four main tabs: **Findings**, **Scan History**, **Remediations**, and **GitHub**. The GitHub tab has three sub-tabs rendered by `switchSubTab()`: **Commits** (calls `GET /api/v1/github/apps/{id}/commits`, renders commit history with avatar/sha/message), **Pull Requests** (calls `GET /api/v1/github/apps/{id}/pr-reviews`, shows PR cards with collapsible security findings), and **Security** (GHAS alerts via "Sync GitHub Alerts" button). The GitHub repo link appears as a small SVG icon button (34×34px, absolute-positioned top-right of the app info card) — no text, hover shows org/repo tooltip.
